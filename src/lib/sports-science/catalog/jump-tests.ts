...
export const cmjTest: TestDefinition = {
  ...
  category: "JUMP",
  description: {
    fr: "Test de référence de la puissance des membres inférieurs et marqueur le plus sensible de la fatigue neuromusculaire au quotidien.",
    en: "Reference test of lower limb power and the most sensitive day to day marker of neuromuscular fatigue.",
  },
  protocol: {
    fr: "Mains aux hanches, descente libre jusqu'à environ 90 degrés de flexion de genou puis extension explosive. Trois essais avec 30 secondes de récupération, meilleur essai retenu. Toujours tester à la même heure et après le même échauffement.",
  ...
  equipment: { fr: "Tapis de contact, plateforme de force ou application vidéo", en: "Contact mat, force plate or video app" },
  durationMin: 8,
  ...
  fields: [
    ...
    { key: "flightTime", label: { fr: "Temps de vol", en: "Flight time" }, unit: "s", type: "number", step: 0.001, min: 0.2, max: 1, optional: true, group: { fr: "Mesure principale", en: "Main measure" }, help: { fr: "À renseigner si la hauteur n'est pas fournie directement.", en: "Fill in if jump height is not provided directly." } },
    { key: "timeToTakeoff", label: { fr: "Temps de mise en action", en: "Time to takeoff" }, unit: "s", type: "number", step: 0.001, min: 0.3, max: 2, optional: true, group: { fr: "Analyse avancée", en: "Advanced analysis" }, help: { fr: "Permet de calculer le RSI modifié. Disponible sur plateforme de force.", en: "Used to compute the modified RSI. Available on force plates." } },
    { key: "sjHeight", label: { fr: "Hauteur du squat jump associe", en: "Associated squat jump height" }, unit: "cm", type: "number", step: 0.1, min: 10, max: 90, optional: true, group: { fr: "Analyse avancée", en: "Advanced analysis" }, help: { fr: "Permet de calculer le ratio d'utilisation du cycle étirement détente.", en: "Used to compute the eccentric utilisation ratio." } },
    { key: "leftHeight", label: { fr: "Saut unilatéral gauche", en: "Left single leg jump" }, unit: "cm", type: "number", step: 0.1, min: 5, max: 60, optional: true, group: { fr: "Unilatéral", en: "Single leg" } },
    { key: "rightHeight", label: { fr: "Saut unilatéral droit", en: "Right single leg jump" }, unit: "cm", type: "number", step: 0.1, min: 5, max: 60, optional: true, group: { fr: "Unilatéral", en: "Single leg" } },
  ],
  compute: (raw, ctx) => {
...
export const squatJumpTest: TestDefinition = {
  ...
  category: "JUMP",
  description: {
    fr: "Saut départ statique sans contre mouvement. Isole la force concentrique pure des membres inférieurs.",
    en: "Jump from a static position without countermovement. Isolates pure concentric lower limb force.",
  },
  protocol: {
    fr: "Position accroupie à 90 degrés de flexion de genou maintenue trois secondes, mains aux hanches, extension explosive sans reprise d'élan. Tout contre mouvement invalide l'essai.",
  ...
  compute: (raw, ctx) => {
    ...
    return {
      ...
      summary: {
        fr: "À comparer au saut avec contre mouvement pour évaluer l'exploitation du cycle étirement détente.",
...
export const dropJumpTest: TestDefinition = {
  key: "drop_jump",
  name: { fr: "Drop jump et indice de force réactive", en: "Drop jump and reactive strength index" },
  shortName: { fr: "Drop jump", en: "Drop jump" },
  category: "JUMP",
  description: {
    fr: "Saut après chute d'une hauteur donnée. Mesure la raideur musculo tendineuse et la capacité à produire de la force en un temps très court.",
  ...
  protocol: {
    fr: "Le joueur se laisse tomber de la boîte sans sauter vers le haut ni vers le bas, puis rebondit le plus vite et le plus haut possible. Consigne : minimiser le temps de contact au sol. Trois essais par hauteur.",
  ...
  equipment: { fr: "Boîte réglable, tapis de contact", en: "Adjustable box, contact mat" },
  durationMin: 10,
...
export const hopTest: TestDefinition = {
  key: "hop_tests",
  name: { fr: "Batterie de sauts unilatéraux", en: "Single leg hop battery" },
  shortName: { fr: "Hop tests", en: "Hop tests" },
  category: "JUMP",
  description: {
    fr: "Quatre sauts à cloche pied qui servent de critères objectifs de retour au jeu après blessure du membre inférieur.",
    en: "Four single leg hop tests used as objective return to play criteria after lower limb injury.",
  },
  protocol: {
    fr: "Mains libres, réception stabilisée trois secondes sans poser l'autre pied. Deux essais valides par jambe et par test. Toujours commencer par le côté sain.",
  ...
  equipment: { fr: "Mètre ruban, chronomètre, couloir de 8 mètres", en: "Tape measure, stopwatch, 8 m lane" },
  durationMin: 15,
  ...
  fields: [
    ...
    { key: "crossoverLeft", label: { fr: "Triple saut croisé gauche", en: "Crossover hop left" }, unit: "cm", type: "number", step: 1, min: 100, max: 900, optional: true, group: { fr: "Triple saut croisé", en: "Crossover hop" } },
    { key: "crossoverRight", label: { fr: "Triple saut croisé droit", en: "Crossover hop right" }, unit: "cm", type: "number", step: 1, min: 100, max: 900, optional: true, group: { fr: "Triple saut croisé", en: "Crossover hop" } },
    { key: "timedLeft", label: { fr: "Saut chronomètre 6 m gauche", en: "6 m timed hop left" }, unit: "s", type: "number", step: 0.01, min: 1, max: 10, optional: true, group: { fr: "Saut chronomètre", en: "Timed hop" } },
    { key: "timedRight", label: { fr: "Saut chronomètre 6 m droit", en: "6 m timed hop right" }, unit: "s", type: "number", step: 0.01, min: 1, max: 10, optional: true, group: { fr: "Saut chronomètre", en: "Timed hop" } },
    { key: "threshold", label: { fr: "Seuil de symétrie exige", en: "Required symmetry threshold" }, unit: "%", type: "number", step: 1, min: 80, max: 100, optional: true, help: { fr: "90% par défaut, 95% après reconstruction du ligament croisé antérieur.", en: "90% by default, 95% after anterior cruciate ligament reconstruction." } },
  ],
  compute: (raw) => {
